...
<script type="text/javascript">
    ...
    $(function() {
        userDataGrid = $('#userDataGrid').datagrid({
            url : '${path }/dataImport/notFoundQuery',
            fit : true,
            striped : true,
            rownumbers : true,
            pagination : true,
    ...
    function searchUserFun() {
       var staTime=$('#st');
        var endTime=$('#ed');
        if (staTime==""||endTime==""){
            parent.$.messager.alert('提示', "时间不能为空", 'info');
            return
        }
        if (staTime<endTime){
            parent.$.messager.alert('提示',"开始时间不得小于结束时间", 'info');
        }
        ...
        <form id="searchUserForm">
            <table>
                <tr>
                    <th>离院时间:</th>
                    <td>
                        <input id="st" name="boStTime" placeholder="点击选择时间" onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" readonly="readonly" />至
                        <input id="ed" name="boEdTime" placeholder="点击选择时间" onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" readonly="readonly" />
