...
/**
 * @description：AOP 日志
 * @author：zhixuan.wang @date：2015/10/28 18:04
 */
@Aspect
@Component
@Order
public class SysLogAspect {
	...
	public Object recordSysLog(ProceedingJoinPoint point) throws Throwable {
		...
		// 非系统账号登陆时才打印日志
		if (bfParams.indexOf("username=system") < 0 && bfParams.indexOf("\"u\":\"system\"") < 0) {
			LOGGER.info(strMessage);
		}
		if (StringUtils.isNotBlank(strMessage) && strMessage.length() > 999) {
	...
	private boolean isWriteLog(String method) {
		String[] pattern = { "postLogin", "loginPost", "logout", "add", "edit", "delete", "grant" };
