...
public class PageInfo {
    ...
    public PageInfo(Map<String, Object> map) {
        ...
        if (pageObj != null && StringUtils.isNumeric(pageObj.toString())) {
            nowpage = Integer.valueOf(pageObj.toString());
            map.remove("page");
        }
        if (rowsObj != null && StringUtils.isNumeric(rowsObj.toString())) {
            pagesize = Integer.valueOf(rowsObj.toString());
            map.remove("rows");
        }
        // 计算当前页
        if (nowpage < 0) {
