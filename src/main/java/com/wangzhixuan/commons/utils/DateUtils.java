...
public class DateUtils {
	...
	public static Date parse(String date) {
		try {
			...
			logger.warn("字符串转世间异常,字符串为{}", date);
			return null;
		}
	}
	...
	public static Date getSomeEndMonthDate(Date date){
		GregorianCalendar gc = (GregorianCalendar) Calendar.getInstance();
		gc.setTime(date);
		Calendar calendar = Calendar.getInstance();
		calendar.set(Integer.valueOf(gc.get(1)), gc.get(2) + 1, 1);
		int day = calendar.getActualMaximum(5);
		calendar.set(Integer.valueOf(gc.get(1)), gc.get(2) + 1, day);
		return calendar.getTime();
	}
	/**
	 * 获取今年是哪一年
	 ...
	 * @return
	 */
	public static Date getSomeDateEndDate(Date date){
		Calendar calendar = Calendar.getInstance();
		if (null !=date){
			calendar.setTime(date);
		}
	...
	public static Map<String,Object> getEndTimeByStaTime(Map<String,Object> map){
		if (map.get("staTime")!=null&&map.get("staTime")!=""){
			Date date=DateUtils.parse(map.get("staTime").toString());
			map.put("endTime", org.apache.commons.lang3.time.DateUtils.addMonths(date,1));
		}else {
			throw new SysException("未选择开始时间");
		}
		return map;
	}
}
